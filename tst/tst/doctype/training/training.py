...
@frappe.whitelist()
def create_new_session(training_name, session_date, session_time, trainer, subject, session_type,compensated_session= None,reason=None, session_row_name=None):
	"""
	...
	if not existing_row:
		...
		new_row = training_doc.append("training_schedule", {
			"date": session_date,
			"reason":reason,
		...
		if compensated_session:
			...
			old_session_name = frappe.get_cached_value("Training Schedule", {"date":compensated_session,"parent":training_name}, "name")
	...
	# get trainer name
	trainer_name = frappe.get_cached_value("Employee",{"name":trainer},"employee_name")
	...
	if session.name:
		frappe.db.sql("""
			...
			WHERE name = %s
		""", ("Created", trainer,trainer_name, session_type, session.name, session_row_name))
		frappe.db.commit()
